...
namespace TicketToRide.Classes
{
    public class Player
    {
        ...
        public void TakeTurn()
        {
            ...
            // If there are no desired routes, then all routes the player wants are already claimed (whether by this player or someone else)
            ...
            // It is possible for a player to have a lot of train cards. So let's have them claim the longest route they can claim with the cards they have available, if they have more than 24 cards
            if (Hand.Count >= 24)
        ...
        private List<BoardRoute> CalculateTargetedRoutes(DestinationCard card)
        {
            ...
            foreach (var route in allRoutes)
            {
                var matchingRoutes = Board.Routes.Routes
                    .Where(x => x.Length == route.Length && x.IsOccupied && x.OccupyingPlayerColor == Color && 
        ...
        private void CalculateTargetedRoutes()
        {
            var allRoutes = new List<BoardRoute>();
        ...
        private bool ClaimRoute(BoardRoute route, TrainColor color)
        {
        ...
        private void ClaimLongestUnclaimedRoute()
        {
            // Find the color whe have the most of, so long as it isn't one of the desired colors
            var mostPopularColor = Hand.GetMostPopularColor(DesiredColors);

            // Find a route for that color
            var matchingRoute = Board.Routes.Routes
                .Where(x => !x.IsOccupied && (x.Color == mostPopularColor || x.Color == TrainColor.Grey))
